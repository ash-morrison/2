...
(function(){
  const title = document.title.replace(' | Publishers Weekly (Demo)','');
  ...
  function openShareDialog(){ if (!dialogBackdrop) return; dialogUrlInput.value = url; if (dialogEmail){ dialogEmail.setAttribute('href', `mailto:?subject=${encodeURIComponent('Sharing: ' + title)}&body=${encodeURIComponent(title + '\n\n' + url)}`); } dialogBackdrop.style.display='flex'; dialogUrlInput && dialogUrlInput.focus(); document.addEventListener('keydown', trapEscape); }
